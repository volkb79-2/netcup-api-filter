...
**This project enforces strict fail-fast configuration: NO DEFAULTS, NO FALLBACKS.**
...
4. **Explicit dependencies** - Variables must be intentionally set at top level

### Agent Workflow

When you encounter a fail-fast error:
...
2. **Read error message** → Follow the hint
   - "check .env.defaults" → Look in `.env.defaults`
   - "source .env.workspace" → Run `source .env.workspace`
   - "run: ./detect-fqdn.sh" → Execute the command

3. **Apply fix** → Set the variable at the correct level
   - Add to `.env.defaults` if project-wide
   - Regenerate `.env.workspace` if environment-specific
...
Variables must be set at the appropriate level:

1. **`.env.defaults`** - Project-wide defaults (version-controlled)
2. **`.env.workspace`** - Environment-specific (auto-generated by post-create.sh)
3. **`.env.services`** - Service naming (centralized registry)
4. **`tooling/{service}/.env`** - Service-specific configuration

See [Fail-Fast Principle](docs/FAIL_FAST_PRINCIPLE.md) for complete documentation and examples.
