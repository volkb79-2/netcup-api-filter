...
async def capture_admin_pages(browser):
    ...
    # Read password from deployment env file (NO DEFAULTS - fail-fast policy)
    # Require REPO_ROOT for portable paths
    repo_root = os.environ.get('REPO_ROOT')
    if not repo_root:
        raise RuntimeError("REPO_ROOT must be set (source .env.workspace or export manually)")
    
    # Require DEPLOYMENT_ENV_FILE to be explicit
    env_file = os.environ.get('DEPLOYMENT_ENV_FILE')
    if not env_file:
        # Try auto-detect but warn loudly
        env_local_path = Path(repo_root) / ".env.local"
        env_webhosting_path = Path(repo_root) / ".env.webhosting"
        if env_local_path.exists():
            env_file = str(env_local_path)
            print(f"⚠️  WARNING: DEPLOYMENT_ENV_FILE not set, auto-detected: {env_file}")
            print(f"⚠️  Set explicitly: export DEPLOYMENT_ENV_FILE={env_file}")
        elif env_webhosting_path.exists():
            env_file = str(env_webhosting_path)
            print(f"⚠️  WARNING: DEPLOYMENT_ENV_FILE not set, auto-detected: {env_file}")
            print(f"⚠️  Set explicitly: export DEPLOYMENT_ENV_FILE={env_file}")
        else:
            raise RuntimeError("DEPLOYMENT_ENV_FILE not set and no .env.local or .env.webhosting found in REPO_ROOT")
    
    if not Path(env_file).exists():
        raise RuntimeError(f"Deployment env file not found: {env_file}")
    ...
    with open(env_file, 'r') as f:
        for line in f:
            if line.strip() and not line.startswith('#') and '=' in line:
                ...
                if key == 'DEPLOYED_ADMIN_PASSWORD':
                    admin_password = value
                    print(f"   Using password from deployment env (length: {len(admin_password)})")
    
    if not admin_password:
        raise RuntimeError(f"DEPLOYED_ADMIN_PASSWORD not found in {env_file}. Run authentication test first.")
...
async def capture_client_pages(browser):
    ...
    # Read demo clients from build_info.json (NO DEFAULTS - fail-fast policy)
    repo_root = os.environ.get('REPO_ROOT')
    if not repo_root:
        raise RuntimeError("REPO_ROOT must be set (source .env.workspace or export manually)")
